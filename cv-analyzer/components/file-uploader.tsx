...
export function FileUploader() {
  ...
  return (
    <div className="w-full max-w-3xl mx-auto flex flex-col">
      ...
      <div className="mb-6">
        <p className="text-lg font-medium text-gray-700 mb-3">{translations.selectLanguage[language]}</p>
        <div className="flex space-x-3">
          <button
            type="button"
            onClick={() => setAiLanguage("en")}
            className={`px-6 py-3 text-lg rounded-md transition-all duration-200 ${
              aiLanguage === "en"
      ...
      <div className="mb-6">
        <button
          ...
          className="flex items-center text-lg text-blue-600 hover:text-blue-800 transition-colors duration-200"
        >
          <svg 
            xmlns="http://www.w3.org/2000/svg" 
            className={`h-6 w-6 mr-2 transform transition-transform duration-300 ${showJobDesc ? 'rotate-90' : ''}`} 
            fill="none" 
            viewBox="0 0 24 24" 
            stroke="currentColor"
          >
      ...
      >
        <div className="pt-2">
          <label htmlFor="job-description" className="block text-lg font-medium text-gray-700 mb-3">
          ...
          <textarea
            ...
            rows={6}
            className="w-full px-4 py-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 text-lg"
      ...
      {/* File uploader */}
      <div
        className={`border-2 border-dashed rounded-md p-10 text-center transition-all duration-300 ${
          fileError 
      ...
      >
        {uploading ? (
          <div className="space-y-5">
            <p className="text-gray-500 text-xl">{translations.uploadProgress[language]}</p>
            ...
            <p className="text-lg text-gray-500">{uploadProgress}%</p>
          </div>
        ) : file && uploadSuccess ? (
          <div className="space-y-3">
            ...
            <p className="text-green-600 font-medium text-xl">{translations.uploadSuccess[language]}</p>
            <p className="text-lg text-gray-500">{file.name}</p>
            <p className="text-lg text-gray-500">{language === 'en' ? 'Redirecting to results...' : 'Sonuçlara yönlendiriliyor...'}</p>
          </div>
        ) : (
          <div className="space-y-6">
            ...
            <p className="text-gray-700 text-xl">{translations.dragDrop[language]}</p>
            <p className="text-gray-500 text-lg">{translations.or[language]}</p>
            <div>
              <label htmlFor="file-upload" className="cursor-pointer text-blue-500 hover:text-blue-700 transition-colors duration-200 hover:underline text-xl">
      ...
      {/* File Error Message */}
      {fileError && (
        <div className="mt-3 text-lg text-red-600 transition-opacity duration-200">
          <p>{fileError}</p>
        </div>
      )}
      ...
      {uploadError && (
        <div className="mt-5 p-4 bg-red-50 border border-red-200 rounded-md text-lg text-red-600 transition-opacity duration-200">
          <p className="font-medium mb-2">{language === 'en' ? 'Upload Failed' : 'Yükleme Başarısız'}</p>
          <p>{uploadError}</p>
        </div>
      )}
      ...
      {file && !uploading && !uploadSuccess && (
        <button
          ...
          className="w-full mt-5 bg-blue-500 text-white py-4 px-6 rounded-md hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all duration-200 transform hover:scale-[1.02] active:scale-[0.98] text-xl font-medium"
        >
      ...
      <div className="mt-5 text-base text-gray-500 text-center w-full">
        {language === 'en' 
